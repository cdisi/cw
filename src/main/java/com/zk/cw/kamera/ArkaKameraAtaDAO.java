...
public class ArkaKameraAtaDAO {
	...
	private static final String INSERT = "INSERT INTO arka_kamera_ata (cihaz_id,kamera_cozunurluk_id,diyafram_acikligi_id,piksel_buyuklugu_id) VALUES (?,?,?,?)";
	private static final String UPDATE = "UPDATE arka_kamera_ata SET cihaz_id=?,kamera_cozunurluk_id=?,diyafram_acikligi_id=?,piksel_buyuklugu_id=? WHERE id=?";
	...
	public static ArkaKameraAta findBy(Cihaz cihaz, Integer offset) throws SQLException {
		...
		while (rset.next()){
			arkaKameraAta = new ArkaKameraAta(rset.getInt("id"),rset.getInt("cihaz_id"),rset.getInt("kamera_cozunurluk_id"),rset.getInt("diyafram_acikligi_id"),rset.getInt("piksel_buyuklugu_id"));
		}
